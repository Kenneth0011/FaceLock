...
NT          X_adv = X_adv - grad.detach().sign() * actual_step_size
        else:
...
NT        X_adv = torch.minimum(torch.maximum(X_adv, X - eps), X + eps)
...
Section        X_adv.requires_grad_()
        image = vae(X_adv).sample

        loss = (image).norm()
        grad, = torch.autograd.grad(loss, [X_adv])
Note        X_adv = X_adv - grad.detach().sign() * actual_step_size

        pbar.set_description(f"[Running attack]: Loss {loss.item():.5f} | step size: {actual_step_size:.4}")

s        X_adv = torch.minimum(torch.maximum(X_adv, X - eps), X + eps)
        X_adv.data = torch.clamp(X_adv, min=clamp_min, max=clamp_max)
        X_adv.grad = None
...
    m   loss_cvl = compute_score(image.float(), X.float(), aligner=aligner, fr_model=fr_model)
        loss_encoder = F.mse_loss(latent, clean_latent)
        loss_lpips = lpips_fn(image, X)
        
        # --- 應用修改後的權重 ---
EXAMPLE        loss = -loss_cvl * (1 if i >= iters * 0.35 else 0.0) + loss_encoder * lambda_encoder + loss_lpips * (1 if i > iters * 0.25 else 0.0)
        # ------------------------

        grad, = torch.autograd.grad(loss, [X_adv])
        X_adv = X_adv + grad.detach().sign() * actual_step_size

CSS        X_adv = torch.minimum(torch.maximum(X_adv, X - eps), X + eps)
        X_adv.data = torch.clamp(X_adv, min=clamp_min, max=clamp_max)
        X_adv.grad = None
